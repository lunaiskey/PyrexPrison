package io.github.lunaiskey.pyrexprison.util.nms;
...
public class NBTTags {
    ...
    public static Pair<CurrencyType,BigInteger> getVoucherValue(ItemStack item) {
        CompoundTag itemTag = getPyrexDataCompound(item);
    ...
    public static CompoundTag getPyrexDataCompound(ItemStack item) {
        item = addPyrexDataContainer(item);
        net.minecraft.world.item.ItemStack nmsStack = CraftItemStack.asNMSCopy(item);
        CompoundTag itemTag = nmsStack.getOrCreateTag();
    ...
    public static ItemID getItemID(ItemStack item) {
        try {
            return ItemID.valueOf(getPyrexDataCompound(item).getString("id"));
        } catch (Exception ignored) {
            return null;
        }
    }
