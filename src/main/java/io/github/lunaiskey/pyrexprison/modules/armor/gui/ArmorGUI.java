package io.github.lunaiskey.pyrexprison.modules.armor.gui;
            ...
            case 16 -> {
                if (armorEquiped) {
                    ...
                    for (ItemStack item : p.getInventory().getArmorContents()) {
                        if (item != null) {
                            if (NBTTags.getPyrexDataCompound(item).get("id") != null) {
                                if (!NBTTags.getPyrexDataCompound(item).get("id").getAsString().contains("PYREX_ARMOR_")) {
                                    allSlotsEmpty = false;
                                }
                            } else {
                                allSlotsEmpty = false;
